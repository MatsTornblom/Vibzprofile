...
export function StandardInputBox({ label, className, ...props }: StandardInputBoxProps) {
  return (
    <div>
      {label && (
        <label className="block text-sm text-vibz-textbox-text font-poppins font-bold mb-2">
          {label}
        </label>
      )}
      <input
        {...props}
        className={`w-full bg-vibz-bg-textbox rounded-full px-4 py-3 text-vibz-textbox-text font-poppins
